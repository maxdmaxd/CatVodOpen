...
import { load } from 'cheerio';
    ...
    let jxUrl = ['https://api.cnmcom.com/webcloud/player/anmm.php?url=','https://api.cnmcom.com/webcloud/player/bnmm.php?url=','https://api.cnmcom.com/webcloud/player/cnmm.php?vid='];
...
async function home(filter) {
    ...
    async function play(inReq, _outResp) {
        ...
        for(const n of $('script')) {
            ...
            if($(n).text().indexOf("video src=\"") >= 0) {
                let url = $(n).text().split("video src=\"")[1].split("';")[0];
                if(url) {
                    playUrl = url;
                }
                return ({
